...
-- MODULO: AUTENTICACION, CURSOS, ACTIVIDADES, RUBRICAS Y CALIFICACIONES
...
BEGIN
    SELECT grado_semestre INTO semestre_materia
    FROM materias
    ...
    IF semestre_materia IS NULL OR semestre_grupo IS NULL
       OR semestre_materia <> semestre_grupo THEN
        RAISE EXCEPTION 'La materia y el grupo deben pertenecer al mismo semestre';
    END IF;
...
WITH anios AS (
    SELECT generate_series(2020, EXTRACT(YEAR FROM CURRENT_DATE)::INT) AS anio
),
periodos AS (
    SELECT
        anio,
